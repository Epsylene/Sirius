...
namespace Sirius
{
    ...
    class Camera
    {
        ...
        public:
            ...
            /// @brief Constructs a generic camera object at (0,
            /// 0, 1), pointing towards (0, 0, -1) and with up
            /// vector (0, 1, 0)
            Camera();
    ...
    ///
    /// @details This camera is intended to be used only for 2D
    /// rendering purposes. As such, it always points in
    /// negative Z direction and can only translate or, if
    /// specified, rotate around this axis.
    class Camera2D: public Camera
    {
        protected:
            ...
            ///     the 2D camera
            ///
            /// @details The view matrix is a composed of a
            /// rotation matrix around `direction` and a `pos`
            /// translation matrix; the projection matrix is a
            /// `Sirius::ortho()` with the arguments from the
            /// constructor.
            ///
        ...
        public:
            ...
            /// @brief Constructs an orthographic camera2D
            /// pointing towards -Z with left and bottom planes
            /// distances set to -1, right and top to 1, near
            /// and far to -1 and 1)
            Camera2D();

            /// @brief Constructs an orthographic 2D camera2D
            /// pointing towards -Z with the provided arguments
            ///
            ...
            /// @brief Set the 2D camera's rotation
            ///
            /// @param rotation Rotation in the 2D plane the camera
            ///     is looking at
            void setRotation(float rotation);

            /// @brief Set the 2D camera's projection matrix arguments
            ///
            /// @param left Left plane distance
            /// @param right Right plane distance
            /// @param bottom Bottom plane distance
    ...
    ///
    /// @details This camera is intended for general 3D usage:
    /// it is a classical first-person camera, used to roam
    /// around the scene and watch the objects in it from
    /// different angles.
    class Camera3D: public Camera
    {
        protected:
            ...
            /// of the 3D camera.
            ///
            /// The view matrix is a `Sirius::lookAt()` with
            /// `pos` as eye, `pos + direction` as center and
            /// `up` as up. The projection matrix is a
            /// `Sirius::perspective()` that takes the arguments
        ...
        public:
            ...
            /// @brief Constructs a default perspective 3D
            /// camera (90º FOV, 16/9 aspect ratio, distances to
            /// near and far planes set to 0.1 and a 100).
            Camera3D();
